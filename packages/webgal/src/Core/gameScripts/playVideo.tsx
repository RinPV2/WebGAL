...
export const playVideo = (sentence: ISentence): IPerform => {
  ...
  console.log('playVideo', sentence);
  if (sentence.content === '') {
  ...
  let bg = getSentenceArgByKey(sentence, 'bg');
  let isBg = false;
  if (bg) {
    isBg = true;
  }
  ...
  // eslint-disable-next-line react/no-deprecated
  ReactDOM.render(
    <div className={styles.videoContainer} style={{ zIndex: isBg ? 4 : 11 }}>
      <video className={styles.fullScreen_video} id="playVideoElement" disablePictureInPicture={true} src={sentence.content} autoPlay={true} loop={isBg} muted={isBg} />
    </div>,
